...
return (colors[idx]);
  ...
  function createBubble() {
    var ns= "http://www.w3.org/2000/svg",
        ...
        hearts= document.createElementNS(ns, "svg"),
        heartsPath= document.createElementNS(ns, "path");
    
    hearts.setAttribute("viewBox", "0 0 600 500")
    heartsPath.setAttribute("d", "M300,150 C500,10 600,300 300,400 C0,300 100,10 300,150");
    bubble.classList.add("bubble");
    bubble.style.color= getRandomColor();
    bubbleDummy.classList.add("bubble-dummy");
    hearts.classList.add("hearts");
    
    hearts.appendChild(heartsPath);
    bubble.appendChild(bubbleDummy);
    bubble.appendChild(hearts);
    ...
    return {
      ...
      _animate: function() {
        var animateBubble= animateIt(bubbleDummy, 1200),
            animatehearts= animateIt(hearts, 2000);  
  ...
  function handleDown(e) {
   ...
   var animation= bubble._animate(),
       totalDelay= animation.bubbleDur+ animation.heartsDur;
    if(e.type) {
